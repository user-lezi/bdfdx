...
exports.default = (0, apiRoute_1.createAPIRoute)({
    meta: {
        ...
        description: "Downloads an image and extracts dominant, palette, and accent colors using pixel sampling.",
        category: "utility",
        tags: ["utility", "image"],
        query: {
            image: {
    ...
    async callback(ctx) {
        const imageUrl = ctx.req.query.image;
        if (!imageUrl) {
            return ctx.res.status(400).json({
                error: "Missing required query parameter: image",
            });
        }
        const img = await (0, canvas_1.loadImage)(imageUrl);
        // Downscale for speed
        const size = 128;
        ...
        const average_color = toColor(Math.floor(average[0] / averageLength), Math.floor(average[1] / averageLength), Math.floor(average[2] / averageLength));
        return ctx.res.json({
            dominant_color,
            palette,
            accent,
