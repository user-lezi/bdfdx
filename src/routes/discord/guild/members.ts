...
export default createAPIRoute({
  meta: {
    ...
    query: {
      fetch: {
        type: "boolean",
        description:
          "Force-fetch all members from Discord API instead of using cache.",
        required: false,
        example: false,
      },
      type: {
  ...
  async callback(ctx) {
    ...
    if (!guild) return ctx.res.status(404).json({ error: "Guild not found." });
    ...
    if (type === "human") members = members.filter((m) => !m.user.bot);
